...
| Item | Status | Notes |
|---|---|---|
| Encryption at rest | 🟡 | Azure-managed disk + database encryption (AES-256) covers everything. On top of that, the SA ID number is field-encrypted (see below). **Everything else — patient name, date of birth, membership number, clinical notes, GPS trail and all five signatures — is plaintext in the database.** Say that explicitly whenever you say "encrypted" |
| Field-level encryption of patient IDs | ✅ | **SHIPPED AND CONVERTED 2026-08-02.** Fernet (AES-128-CBC + HMAC) on `cases.patient_id_number` and on `patient_id_number` inside `digital_prfs.form_data`, via SQLAlchemy TypeDecorators so no call site can bypass it (`app/utils/encrypted_types.py`). Column widened Text; keyed HMAC-SHA256 lookup hash added so equality search still works. Production: 160 records converted, 0 plaintext remaining, all verified reversible. Migrations `c9e4a71f3b28` + `d1f7b93c6e40`. **Re-run `python encrypt_patient_ids.py --verify` before quoting that number to anyone** — the repository cannot prove a runtime conversion ran, and a false statement about a security control to a regulated counterparty is worse than the gap it claims to close. SCOPE: the ID number ONLY. No other field is field-encrypted |
