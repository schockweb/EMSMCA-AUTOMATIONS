...
function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  const navItems = [
    ...
    ...(isAdmin ? [
      // NOTE: there is deliberately NO "Failed Forms" page/nav item. Failed or
      // stuck PRFs surface as the amber alert on Case Management, whose popup
      // explains each problem and offers the fix (Retry) inline.
    ] : []),
  ];

  const visibleNav = navItems.filter(item => isAdmin || userPerms.includes(item.perm));

  // Poll for locked-out provider/crew accounts so the Clients tab can flag them
  // with a red "!" — the admin then handles the unlock from the Clients page.
  const canSeeClients = isAdmin || userPerms.includes('providers');
  const [lockedCount, setLockedCount] = useState(0);
  useEffect(() => {
    if (!canSeeClients) return;
    let alive = true;
    const load = () => api.get('/api/account-security/locked')
      .then(r => { if (alive) setLockedCount((r.data || []).length); })
